...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleValidation(MethodArgumentNotValidException ex, HttpServletRequest request) {
        ...
        return buildResponse(HttpStatus.BAD_REQUEST, "Error de validación", request.getRequestURI(), errors);
    }

    @ExceptionHandler(DataIntegrityViolationException.class)
    public ResponseEntity<ApiError> handleDataIntegrity(DataIntegrityViolationException ex, HttpServletRequest request) {
        return buildResponse(HttpStatus.CONFLICT, "Conflicto de integridad de datos. Verifica restricciones únicas o referencias.", request.getRequestURI(), null);
    }
